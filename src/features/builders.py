...
def build_market_features(
    ...
    if feature_flags.get("compression", False):
        ...
        out["compression_bb_width_20"] = (4.0 * close_std_20) / close_ma_20.replace(0, np.nan)

        range_1 = (out["high"] - out["low"]) / out["close"].replace(0, np.nan)
        range_avg_5 = grouped.apply(lambda g: range_1.loc[g.index].rolling(5).mean()).reset_index(
            level=0, drop=True
        )
        range_avg_20 = grouped.apply(lambda g: range_1.loc[g.index].rolling(20).mean()).reset_index(
            level=0, drop=True
        )
        out["compression_range_ratio_5_20"] = range_avg_5 / range_avg_20.replace(0, np.nan)

        if "atr_14" in out.columns:
            atr_avg_20 = grouped["atr_14"].transform(lambda s: s.rolling(20).mean())
            out["compression_atr_ratio_14_20"] = out["atr_14"] / atr_avg_20.replace(0, np.nan)
    ...
    if feature_flags.get("range_position", False):
        ...
        out["range_pos_20"] = (out["close"] - low_20) / (high_20 - low_20).replace(0, np.nan)
        out["range_dist_high_20"] = out["close"] / high_20.replace(0, np.nan) - 1.0
        out["range_dist_low_20"] = out["close"] / low_20.replace(0, np.nan) - 1.0

    # --------------------------------
    # Trend quality pack
    # --------------------------------
    if feature_flags.get("trend_quality", False):
        close_diff = grouped["close"].transform(lambda s: s.diff().abs())
        path_len_20 = grouped.apply(lambda g: close_diff.loc[g.index].rolling(20).sum()).reset_index(
            level=0, drop=True
        )
        net_move_20 = grouped["close"].transform(lambda s: (s - s.shift(20)).abs())
        out["trend_efficiency_20"] = net_move_20 / path_len_20.replace(0, np.nan)

        if "ema_spread_12_26" in out.columns and "atr_14" in out.columns:
            out["trend_quality_ema_over_atr"] = out["ema_spread_12_26"] / out["atr_14"].replace(0, np.nan)

        out["trend_slope_10"] = grouped["close"].transform(lambda s: (s - s.shift(10)) / 10.0)
    ...
    if feature_flags.get("market_context", False):
        ...
        if "ret_1" in out.columns:
            out["mkt_mean_ret_1"] = by_ts["ret_1"].transform("mean")
            out["mkt_median_ret_1"] = by_ts["ret_1"].transform("median")
            out["rel_ret_1_vs_mkt"] = out["ret_1"] - out["mkt_mean_ret_1"]
        if "rv_20" in out.columns:
            out["mkt_mean_rv_20"] = by_ts["rv_20"].transform("mean")
            out["rel_rv_20_vs_mkt"] = out["rv_20"] - out["mkt_mean_rv_20"]
        out["mkt_volume_rank"] = by_ts["volume"].rank(pct=True)
